...
class PerSchedule extends React.Component {
  ...
  componentDidUpdate = (prevProps, prevState) => {
    ...
    const { nrHolidayList, render } = this.state;

    if (user.id && prevProps.user.id !== user.id) {
      this.loadPending();
    }

    // Load new days when nrHolidayList is available and render is false
    if (
      nrHolidayList.length > 0 &&
      !render &&
      prevState.nrHolidayList.length === 0
    ) {
  ...
  loadEntries = () => {
    fetch("https://secure-earth-82827.herokuapp.com/sked/entries")
      ...
      .then((entries) =>
        this.setState({
          entries: entries.filter((entry) => entry.isactive === true),
        })
      );
  };
  ...
  assignOrDelete = (typeId, day = this.state.day) => {
    ...
    if (typeId !== -1) {
      ...
      const date =
        dateContext.format("MM") + "/" + day + "/" + dateContext.format("YYYY");
      if (user.isadmin) {
        let method = "post";
          ...
          for (let n = 0; n < pending[j].dates.length; n++) {
            if (pending[j].dates[n] === date) {
              this.deleteCall(
                pending[j].entryid,
                date,
                this.state.activeDocs[this.state.docIndex].id
              );
              break;
            }
          }
            ...
            >
              {({ blob, url, loading, error }) =>
                loading ? (
                  ...
                  <span onMouseOver={this.hoverSpan}>Download as PDF</span>
                )
              }
            </PDFDownloadLink>
          </Col>
          ...
          <Modal show={show} onHide={this.toggleShow}>
            ...
            <Form>
              ...
              <Modal.Footer>
                ...
                <Button
                  onClick={() => this.assignOrDelete(this.state.radio)}
                  variant="primary"
                >
                  Submit
                </Button>
